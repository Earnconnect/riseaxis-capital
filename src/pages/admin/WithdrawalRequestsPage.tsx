...
const statusStyle = (s: string): React.CSSProperties => {
  if (s === 'completed') return { background: '#F5F3FF', color: '#7C3AED', border: '1px solid #DDD6FE' }
            ...
            <div className="md:hidden">
              {txns.map((t, i) => (
                <div key={t.id} className="px-4 py-4"
                  ...
                  {t.status === 'approved' && (
                    <button onClick={() => markCompleted(t)} disabled={actionId === t.id}
                      className="w-full flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-bold text-white transition-all hover:brightness-105 disabled:opacity-60"
                      style={{ background: 'linear-gradient(135deg, #7C3AED, #6D28D9)' }}>
                      {actionId === t.id ? <Loader2 size={12} className="animate-spin" /> : <CheckCircle2 size={12} />} Mark ACH Completed
                    </button>
                  )}
                </div>
              ))}
                      ...
                      <td className="px-5 py-3.5">
                        ...
                        {t.status === 'approved' && (
                          <button onClick={() => markCompleted(t)} disabled={actionId === t.id}
                            className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold text-white transition-all hover:brightness-105 disabled:opacity-60"
                            style={{ background: 'linear-gradient(135deg, #7C3AED, #6D28D9)' }}>
                            {actionId === t.id ? <Loader2 size={11} className="animate-spin" /> : <CheckCircle2 size={11} />} Mark ACH Completed
                          </button>
                        )}
                      </td>
                    </tr>
